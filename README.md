...
| 역할 | GitHub | Role |
|---|---|---|
| PM | wefefwf | PM(팀장), GitHub 통합 관리, DB 부관리자, PPT 제작, UI 설계서 제작 |
| PL | jpark281 | PL(기술고문), Class Diagram 제작 |
| Member | Hyein-Park-git | 서기, 테이블 정의서 제작 |
| Member | devDuck-9 | PPT 제작, UI 설계서 제작 |
| Member | 5x5cube | 요구사항 명세서 제작 |
| Member | osjqa0918 | ERD 제작, DB 관리자 |

## 담당 기능

| GitHub | 담당 기능 |
|---|---|
| wefefwf | 메인 페이지(+로고 디자인), KIT SHOP, 포트폴리오(+후기 기능), 내 일정관리 |
| jpark281 | 의뢰글 리스트(+카테고리), 내 의뢰 목록 조회, 의뢰글 상태 표시, 선택 낙찰, 관리자 버튼 |
| Hyein-Park-git | 결제 폼(+결제), 장바구니(+결제), 구매 내역 확인(유저), 주문 내역 확인(업체), 배송 상태 변경, 수익 통계/정산 |
| devDuck-9 | 업체 입찰 제안, 내 제안, 유저 선정 제안 댓글(+버튼), 유저 업체 비교, 미선정 댓글 내리기, 관리자 버튼 |
| 5x5cube | 로그인, 회원가입, 정보 수정, 관리자 권한, 공지사항 |
| osjqa0918 | 1:1 채팅, 내 채팅, 플로팅 챗 버튼(챗봇), 플로팅 챗 버튼(진행 중인 채팅) |
